...
class Simulation:
    ...
    def __init__(self, bisimulation, candidate_element_tuple):
        ...
        self.simulator_transitions = set()
    ...
    def is_able_to_simulate(self):
        ...
        while is_able and i < len(simulated_transitions):
            ...
            self._set_simulator_transitions()
            ...
            is_able = self._exists_a_valid_transition_subset_that_simulates(cleaned_simulated_knowledge, cleaned_simulator_knowledge)

            i += 1

        return is_able

    def _set_simulator_transitions(self):
        self.simulator_transitions = self.simulator_state.get_transitions_with(self.simulated_transition.label)

    def _exists_a_valid_transition_subset_that_simulates(self, cleaned_simulated_knowledge, cleaned_simulator_knowledge):
        ...
        simulator_transitions_subsets = list(powerset(self.simulator_transitions))
        ...
        while (not valid_transitions_set_exists) and j < len(simulator_transitions_subsets):
            ...
            valid_transitions_set_exists = self._transitions_subset_fall_into_relation(simulator_transitions_subset, cleaned_simulated_knowledge, cleaned_simulator_knowledge) and \
    ...
    def _is_able_to_simulate_knowledge(self, simulator_transitions_subset, cleaned_simulated_knowledge, cleaned_simulator_knowledge):
        ...
        simulator_knowledge = cleaned_simulator_knowledge.add(
            Assertion(Or({transition.assertion.expression for transition in simulator_transitions_subset}))
        )
